...
if (isset($_POST['save_category'])) {
    ...
    try {
        ...
        if ($stmt->execute()) {
            $feedback = ['type' => 'success', 'msg' => "Archive category successfully $action."];
        } else {
...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        if ($stmt->execute()) {
            $feedback = ['type' => 'success', 'msg' => "Category purged from system."];
        }
    } catch (mysqli_sql_exception $e) {
        $feedback = ['type' => 'error', 'msg' => "Cannot delete: This category is currently linked to active documents or roles."];
    }
}
    ...
    <main class="flex-1 p-8 lg:p-12">
        ...
        <div class="flex justify-between items-end mb-10">
            ...
            <button onclick="openModal()" class="crimson-btn text-white px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2">
                <i class="fa-solid fa-plus"></i> Initialize New Category
            </button>
        </div>

        <!-- System Alerts -->
        <?php if($feedback['msg']): ?>
            <div class="animate__animated animate__shakeX mb-6 p-4 rounded-2xl flex items-center gap-3 <?php echo $feedback['type'] == 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-rose-50 text-rose-700 border border-rose-100'; ?>">
                <i class="fa-solid <?php echo $feedback['type'] == 'success' ? 'fa-circle-check' : 'fa-triangle-exclamation'; ?>"></i>
                <span class="text-xs font-bold uppercase tracking-tight"><?php echo $feedback['msg']; ?></span>
            </div>
        <?php endif; ?>

        <!-- Table View -->
        <div class="glass-panel rounded-[2rem] shadow-sm overflow-hidden animate__animated animate__fadeInUp">
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-900 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                        <th class="px-8 py-5">System ID</th>
                        <th class="px-8 py-5">Category Designation</th>
                        <th class="px-8 py-5">Description Profile</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="px-8 py-6 text-right space-x-2">
                            <button onclick='editCategory(<?php echo json_encode($row); ?>)' class="w-9 h-9 rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-900 hover:text-white transition shadow-sm">
                                <i class="fa-solid fa-pen-to-square text-xs"></i>
                            </button>
                            <a href="" onclick="return confirm('WARNING: Permanent data purge. Continue?')" class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition shadow-sm">
                ...
                <form action="" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="category_name" id="category_name" required
                            class="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-rose-500 focus:bg-white outline-none font-bold text-slate-800 transition">
                    </div>

                    <div>
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Description</label>
                        <textarea name="description" id="description" rows="4"
                            class="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-100 focus:border-rose-500 focus:bg-white outline-none font-semibold text-slate-500 text-sm transition"></textarea>
                    </div>

                    <button type="submit" name="save_category" class="crimson-btn w-full py-5 rounded-2xl text-white font-black uppercase tracking-[0.2em] text-xs">
                        Commit to Database
                    </button>
                </form>
            </div>
